...
function generateIndex() {
  ...
  try {
    files = readdirSync(DOCS)
      .filter((f) => f.startsWith("emdr-") && f.endsWith(".html"))
      .sort()
      .reverse();
  } catch {
    files = [];
  ...
  for (const f of files.slice(0, 60)) {
    const date = f.replace("emdr-", "").replace(".html", "");
...
<title>EMDR Mind · EMDR 文獻日報</title>
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .clinic-links { margin-top: 48px; display: flex; flex-direction: column; gap: 10px; }
  .clinic-link { display: flex; align-items: center; gap: 14px; padding: 16px 20px; background: var(--surface); border: 1px solid var(--line); border-radius: 16px; text-decoration: none; color: var(--text); transition: all 0.2s; font-size: 14px; font-weight: 600; }
  .clinic-link:hover { border-color: var(--accent); transform: translateX(4px); }
  .clinic-link .icon { font-size: 22px; flex-shrink: 0; }
