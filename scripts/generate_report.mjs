...
function parseArgs() {
  ...
  const opts = { input: "papers.json", output: "", apiKey: process.env.NVIDIA_API_KEY || "" };
  ...
  try {
    ...
    if (start !== -1 && end > start) {
      try {
        ...
        sub = sub.replace(/"\s*\n/g, '"\n');
        sub = sub.replace(/\bNaN\b/g, "null");
        sub = sub.replace(/\bInfinity\b/g, "null");
        try {
          return JSON.parse(sub);
        } catch {
          const braceCount = (sub.match(/{/g) || []).length;
          const closeCount = (sub.match(/}/g) || []).length;
          if (braceCount > closeCount) {
            sub += "}".repeat(braceCount - closeCount);
          }
          try {
            return JSON.parse(sub);
          } catch (e2) {
            console.error(`[WARN] Final JSON parse attempt failed: ${e2.message}`);
            console.error(`[DEBUG] First 500 chars: ${sub.slice(0, 500)}`);
            return null;
          }
        }
      }
...
{
  "date": "${dateStr}",
  "market_summary": "1-2句話總結今天文獻的整體趨勢與亮點（聚焦 EMDR 與創傷治療）",
  "top_picks": [
    {
      "rank": 1,
      "title_zh": "中文標題",
...
每篇 paper 的 tags 請從以下選擇：EMDR、PTSD、創傷治療、雙側刺激、工作記憶、解離症、複雜性PTSD、憂鬱症、焦慮症、兒少創傷、難民心理健康、社區心理衛生、神經生物學、系統性回顧、隨機對照試驗、慢性疼痛、耳鳴、成癮、邊緣性人格障礙、功能神經症、悲傷輔導、道德傷害、親密伴侶暴力、性侵、災難心理健康。
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media (max-width: 600px) { .container { padding: 36px 18px 60px; } .summary-card, .news-card { padding: 20px 18px; } .pico-grid { grid-template-columns: 1fr; } footer { flex-direction: column; gap: 6px; text-align: center; } .topic-name { width: 70px; font-size: 11px; } .clinic-links { flex-direction: column; } }
</style>
</head>
<body>
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>EMDR Mind · EMDR 文獻日報</h1>
      <div class="header-meta">
        <span class="badge badge-date">📅 ${dateDisplay}</span>
        <span class="badge badge-count">📊 ${totalCount} 篇文獻</span>
<span class="badge badge-source">Powered by Multi-Source + NVIDIA AI</span>
      </div>
    </div>
  </header>
...
async function main() {
  ...
  if (!papersData || !papersData.papers?.length) {
    ...
    analysis = await callNvidiaAPI(opts.apiKey, papersData);
    if (!analysis) {
  ...
  const outDir = dirname(resolve(ROOT, opts.output));
  if (!existsSync(outDir)) mkdirSync(outDir, { recursive: true });
  writeFileSync(resolve(ROOT, opts.output), html, "utf-8");
